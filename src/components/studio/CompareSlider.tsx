...
export const CompareSlider: React.FC<{ before: string; after: string; className?: string }> = ({ before, after, className }) => {
  ...
  return (
    ...
    >
      <img src={after} alt="Después" draggable={false} className="absolute inset-0 w-full h-full object-contain" />
      <img
        src={before}
        alt="Antes"
        draggable={false}
      ...
      <span className="absolute top-3 left-3 px-2 py-1 rounded-md bg-black/60 text-[12px] pointer-events-none">Antes</span>
      <span className="absolute top-3 right-3 px-2 py-1 rounded-md bg-black/60 text-[12px] pointer-events-none">Después</span>
    </div>
  );
};
